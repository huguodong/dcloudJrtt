...
var hui = (function(selector, undefined) {
	var isId = /^#([\w-]+)$/,
		...
		isTag = /^[\w-]+$/;
	var huibackNum = 0,
		huiResizeNeedDo = new Array(),
		huiReSizeTimer = null,
		huiDeputes = new Array();
	var huibase = function(selector) {
		if(!selector) {
			return addFuns(Array(document));
		}
		if(typeof(selector) == 'string') {
			selector = selector.trim();
			return getDoms(selector);
		}
	...
	var hcExtends = {
		...
		dblclick: function(callBack) {
			...
			for(var i = 0; i < this.length; i++) {
				this.dom[i].addEventListener('dblclick', callBack);
			}
		},
		...
		touchMove: function(callBack) {
			...
			this.dom[0].addEventListener('touchmove', function(e) {
				if(callBack) {
					callBack({
						x: Math.round(e.changedTouches[0].pageX - _self.touchStartX),
						y: Math.round(e.changedTouches[0].pageY - _self.touchStartY)
					});
				}
			});
		},
		...
		swipe: function(callBack) {
			...
			var _self = this;
			this.dom[0].addEventListener('touchend', function(e) {
				var x = Math.round(e.changedTouches[0].pageX - _self.touchStartX),
					y = Math.round(e.changedTouches[0].pageY - _self.touchStartY);
				var absX = Math.abs(x),
					absY = Math.abs(y);
				if(absX > absY) {
		...
		appendTo: function(parentObj) {
			...
			if(typeof(parentObj) == 'object') {
				...
				if(parentDom.length >= 1) {
					parentDom.dom[0].appendChild(this.dom[0]);
				}
			}
		},
		prependTo: function(parentObj) {
			...
			if(typeof(parentObj) == 'object') {
				parentObj.dom[0].insertBefore(this.dom[0], parentObj.dom[0].firstChild);
			} else if(typeof(parentObj) == 'string') {
				var parentDom = hui(parentObj);
				if(parentDom.length >= 1) {
					parentDom.dom[0].insertBefore(this.dom[0], parentDom.dom[0].firstChild);
				}
			}
		},
		animate: function(animateObj, timer, callBack) {
			...
			var interVal = null,
				styleObj = this.dom[0].style,
				i = 0,
				start = {};
			if(this.dom[0].getAttribute('isAnimate')) {
				return false;
			}
			...
			interVal = setInterval(function() {
				for(var k in animateObj) {
					...
					if(k.indexOf('scroll') != -1) {
						if(startVal != endVal) {
							eval('thisObj.dom[0].' + k + ' = "' + (startVal + (endVal - startVal) * i / timer) + '";');
						}
					} else {
						endVal = endVal.toString();
						if(endVal.indexOf('px') != -1) {
							endVal = Number(endVal.replace('px', ''));
							if(startVal != animateObj[k]) {
								eval('styleObj.' + k + ' = "' + (startVal + (endVal - startVal) * i / timer) + 'px";');
							}
						} else if(endVal.indexOf('%') != -1) {
							endVal = Number(endVal.replace('%', ''));
							if(startVal != animateObj[k]) {
								eval('styleObj.' + k + ' = "' + (startVal + (endVal - startVal) * i / timer) + '%";');
							}
						} else {
							if(startVal != animateObj[k]) {
								eval('styleObj.' + k + ' = "' + (startVal + (endVal - startVal) * i / timer) + '";');
				...
				if(i >= timer) {
					...
					thisObj.dom[0].removeAttribute('isAnimate');
					if(callBack) {
						callBack();
					}
				};
		...
		resetLoadingButton: function() {
			this.html(this.attr('HUI_BTN_RESET'));
		...
		offset: function() {
			...
			return huibase.offset(this.dom[0]);
		},
		isShow: function() {
			if(this.length != 1) {
				return true;
		...
		scrollX: function(num, sonsTag, extraValue) {
			...
			for(var i = 0; i < this.length; i++) {
				var cObj = hui(this.dom[i]),
					sons = cObj.find(sonsTag),
					total = sons.length;
				sons.css({
					'width': (setWitdh + extraValue) + 'px',
					'float': 'left'
				});
				cObj.find('div').eq(0).css({
					width: (setWitdh * total) + 'px'
				});
				cObj.css({
					'overflowX': 'auto'
				});
	...
	huibase.ajax = function(sets) {
		...
		if(typeof(xhr) == 'undefined') {
			if(sets.error) {
				sets.error('无法加载XMLHttpRequest模块！');
			}
			return;
		}
	...
	huibase.getItem = function(keyName) {
		return plus.storage.getItem(keyName);
	};
	huibase.setItem = function(keyName, val) {
		plus.storage.setItem(keyName, val);
	};
	huibase.removeItem = function(keyName) {
		return plus.storage.removeItem(keyName);
	};
	...
	huibase.close = function(vId) {
		var w = plus.webview.getWebviewById(vId);
		if(w) {
			w.close();
		}
	};
	huibase.listViews = function() {
		return plus.webview.all();
	};
	huibase.getView = function(vId) {
		return plus.webview.getWebviewById(vId);
	};
	huibase.getCView = function() {
	...
	huibase.maskHide = function() {
		if(huibase.mask) {
			document.body.removeChild(huibase.mask);
		}
	};
	huibase.maskTap = function(callBack) {
		huibase.mask.addEventListener('click', callBack);
	};
	...
	huibase.scrollTop = function(val) {
	...
	huibase.numberBox = function() {
		...
		for(var i = 0; i < numberBoxes.length; i++) {
			var numberBox = numberBoxes[i],
				numberBoxL = numberBox.getElementsByClassName('reduce')[0];
	...
	huibase.lazyLoad = function(className) {
		...
		window.addEventListener('scroll', function() {
			...
			huibase.timerForLazy = setTimeout(function() {
				huibase.lazyLoadNow(className)
			}, 200);
		});
	...
	huibase.lazyLoadNow = function(className) {
		...
		var winInfo = hui.winInfo(),
			imgs = new Array(),
			lazyObj = hui('.' + className);
		for(var i = 0; i < lazyObj.length; i++) {
			var dom = lazyObj.dom[0],
				realSrc = dom.getAttribute('lazySrc');
			console.log(i + ":" + realSrc)
			var setsY = hui.offset(dom);
			console.log("setsY:" + JSON.stringify(setsY))
			console.log("setsY.top:" + setsY.top)
			console.log("winInfo.height:" + winInfo.height)
			console.log("winInfo.scrollTop:" + winInfo.scrollTop)
	...
	huibase.toast = function(msg, timer) {
		...
		if(huibase.ToastTimer) {
			clearTimeout(huibase.ToastTimer);
		}
		if(timer == 'short') {
			timer = 2000;
		} else {
		...
		huibase.ToastTimer = setTimeout(function() {
			toast.remove();
		}, timer);
	};
	...
	huibase.extend = function(funName, fun) {
		eval('hcExtends.' + funName + ' = fun;');
	};
	huibase.resize = function(callBack) {
...
function huiLog(data) {
	console.log(data);
}

function huiJsonLog(data) {
